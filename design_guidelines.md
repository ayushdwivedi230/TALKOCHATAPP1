...
- Typing indicator: "Username is typing..." with animated dots
  - Runtime events: `typing`, `stop_typing` (client -> server) and `userTyping`, `userStopTyping` (server -> clients).
  - Timing: server treats 3s of no typing as stopped; clients should debounce to avoid spamming events.
