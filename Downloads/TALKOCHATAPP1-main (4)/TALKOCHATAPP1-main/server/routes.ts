...
export async function registerRoutes(app: Express) {
  ...
  // Typing state timers: key = `${userId}:${recipientId || 'group'}`
  const typingTimers = new Map<string, NodeJS.Timeout>();
  const lastTypingAt = new Map<number, number>();
      ...
      try {
        ...
        ws.on("message", async (data) => {
          try {
            ...
            // Simple rate-limit for typing events per user
            const now = Date.now();
            if (message.type === "typing" || message.type === "stop_typing") {
              const last = lastTypingAt.get(userId!) || 0;
              if (now - last < 50) return; // ignore bursts
            ...
            if (message.type === "typing" || message.type === "stop_typing") {
              const recipientId = message.recipientId ?? null; // null => group
              const key = `${userId}:${recipientId ?? 'group'}`;
              ...
              if (message.type === "typing") {
                ...
                if (recipientId === null) {
                  broadcastMessage("userTyping", { userId, recipientId: null });
                } else {
                  const recipientSet = clients.get(recipientId);
                  if (recipientSet) {
                    const payload = JSON.stringify({ type: "userTyping", data: { userId, recipientId } });
                    recipientSet.forEach(c => c.readyState === 1 && c.send(payload));
                  }
                }
                ...
                const t = setTimeout(() => {
                  ...
                  if (recipientId === null) {
                    broadcastMessage("userStopTyping", { userId, recipientId: null });
                  } else {
                    const recipientSet = clients.get(recipientId);
                    if (recipientSet) {
                      const payload = JSON.stringify({ type: "userStopTyping", data: { userId, recipientId } });
                      recipientSet.forEach(c => c.readyState === 1 && c.send(payload));
                    }
                  }
                }, 3000);
        ...
        ws.on("close", () => {
          ...
          // Clean up any typing timers for this user and broadcast stop events
          for (const key of Array.from(typingTimers.keys())) {
            if (key.startsWith(`${userId}:`)) {
              const t = typingTimers.get(key)!;
              clearTimeout(t);
              typingTimers.delete(key);
              // broadcast stop for group or specific recipient
              const parts = key.split(':');
              const recipient = parts[1] === 'group' ? null : Number(parts[1]);
              if (recipient === null) {
                broadcastMessage("userStopTyping", { userId, recipientId: null });
              } else {
                const recipientSet = clients.get(recipient);
                if (recipientSet) {
                  const payload = JSON.stringify({ type: "userStopTyping", data: { userId, recipientId: recipient } });
                  recipientSet.forEach(c => c.readyState === 1 && c.send(payload));
                }
              }
            }
